...
Script for reproducing images used in Extended Data Figure 6
"""
...
model_families_dict = {
    ...
    'CanESM5-CanOE': [],
    'CMCC-CM2-SR5': ['NorESM2-MM', 'TaiESM1'],
    ...
    'MRI-ESM2-0': [],
    'NorESM2-MM': ['CMCC-CM2-SR5','TaiESM1'],
    'TaiESM1': ['CMCC-CM2-SR5','NorESM2-MM'],
...
for idx_take_out, (take_out, take_out_family) in enumerate(model_families_dict.items()):
    models_list_remaining = models_list.copy()

    if idx_take_out < 9:
        shuffle_idx = f'0{idx_take_out+1}'
    else:
        shuffle_idx = f'{idx_take_out+1}'

    model_taken_out = models_list[idx_take_out]
    '''
        Rimuoviamo il model taken out corrente sul quale è stato fatto il TL di tutti gli altri modelli
    '''
    models_list_remaining.remove(model_taken_out)

    if take_out_family:
        for family_member in take_out_family:
            models_list_remaining.remove(family_member)

    pickle_in = open(f'{ROOT_DATA}/Transfer_Learning_on_Simulations_AM_families/Predictions_shuffle-{shuffle_idx}.pickle', 'rb')
...
for idx_short_scenario, short_scenario in enumerate(short_scenarios_list):
    ...
    for pos in xpos:
        ...
        if pos==1:
            axes[idx_short_scenario].fill([left,right,right,left],
                        [lower,lower,upper,upper],
                        facecolor='lightblue',edgecolor='black',linewidth=0.3, label='DNNs')
        else:
            axes[idx_short_scenario].fill([left,right,right,left],
                        [lower,lower,upper,upper],
    ...
    legend = axes[idx_short_scenario].legend(loc='upper left', shadow=False, fontsize='small',ncol=1,frameon=True,facecolor='white', framealpha=1,prop={"family":"Arial", 'size':size_legend})    
...
out_filename = f'Ext_Data_Fig_6.png'

plt.savefig(f'{out_filename}', dpi=300)
plt.close()