...
Script for reproducing images used in Extended Data Fig. 8
"""
...
for scenario_short_idx, scenario_short in enumerate(short_scenarios_list):
    ...
    axs[scenario_short_idx].fill_between(np.arange(start_year_training, end_year_training+1), -2, warming_ensemble_predictions_tl_means[0,:end_year_training-start_year_training+1], color='red', alpha=0.1, zorder = 0)
...
plt.text(x=0.5, y=0.96, s='Global average near surface air temperature anomaly', fontsize=31, ha="center", transform=fig.transFigure)
plt.text(x=0.5, y=0.93, s='Base period: 1850–1900', fontsize=29, ha="center", transform=fig.transFigure)
...
plt.savefig(f'Ext_Data_Fig_8.png', dpi=300, bbox_inches='tight')
plt.close()
