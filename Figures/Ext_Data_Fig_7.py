...
Script for reproducing images used in Extended Data Figure 7
"""
...
ensemble_predictions_means = np.mean(annual_predictions_means, axis=0) # (3, 249)

""" Compute 5-95% for temperatures predicted by the DNNs in 1850-2098 """
q05_predictions = np.zeros((len(short_scenarios_list),249))         # (3,249)
q95_predictions = np.zeros((len(short_scenarios_list),249))         # (3,249)
...
for scenario_short_idx, scenario_short in enumerate(short_scenarios_list):
    ...
    axs[scenario_short_idx].set_title(f'Scenario {scenario}\nRMSE (1850–2022): {round(rmse_scenario[scenario_short_idx],2)} °C',
                                      size=19, linespacing=1.5, pad=20) #, **font)
...
fig.suptitle(f'Model taken out: {model_taken_out} — Training set: 2023–2098 — Test set: 1850–2022', y=0.975, fontsize=24)
...
plt.savefig(f'Ext_Data_Fig_7.png', dpi=300, bbox_inches='tight')
