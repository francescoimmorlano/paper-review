...
for scenario_short_idx, scenario_short in enumerate(short_scenarios_list):
    for model_idx, model in enumerate(models_list):
        axs[scenario_short_idx,0].plot(np.arange(1850, 2099), annual_predictions_means_C[model_idx, scenario_short_idx,:], linewidth=2, linestyle='--', label=f'{model}')
        axs[scenario_short_idx,1].plot(np.arange(1850, 2099), annual_simulations_means_C[model_idx,scenario_short_idx,:], linewidth=1, label=f'{model}')

    scenario = f'SSP{scenario_short[-3]}-{scenario_short[-2]}.{scenario_short[-1]}'
    
    axs[scenario_short_idx,0].set_title(f'DNNs — {scenario}', size=35, pad=15)
    axs[scenario_short_idx,1].set_title(f'CMIP6 — {scenario}', size=35, pad=15)
