...
Script for reproducing images used in Extended Data Figure 5
"""
...
""" 
shuffle_idx = 1 to shuffle_idx = 22
Set shuffle_idx = 9 to reproduce Figure 1 present in the paper
"""
shuffle_idx = '17'
...
pre_train_predictions = np.zeros((len(models_list), 3, n_projections_years, n_lat_points, n_lon_points))
...
predictions_after_tl = np.zeros((len(models_list_take_out), len(short_scenarios_list), n_training_years+n_test_years, n_lat_points, n_lon_points))
...
fig.suptitle(f'Model taken out: {model_taken_out}', size=size_suptitlefig)
...
for idx_short_scenario, short_scenario in enumerate(short_scenarios_list):
    ...
    ax0.set_title('Bias (Pre-trained DNNs - Taken-out ESM)', loc='center', size=size_title_axes, pad=17)
    ax0.coastlines()
    gl0 = ax0.gridlines(crs=ccrs.PlateCarree(), draw_labels=True, linestyle='--', linewidth=0.1, color='black')
    ...
    cs1=ax1.contourf(lons_cyclic,lats,difference_warming_cyclic_data, levels=levels,
                            transform = ccrs.PlateCarree(),
                            cmap=colormap, vmin=vmin, vmax=vmax)
    ax1.set_title('Bias (DNNs after TL - Taken-out ESM)', loc='center', size=size_title_axes, pad=17)
    ax1.coastlines()
    gl1 = ax1.gridlines(crs=ccrs.PlateCarree(), draw_labels=True, linestyle='--', linewidth=0.1, color='black')
...
plt.savefig(f'Ext_Data_Fig_5.png', bbox_inches='tight', dpi=300)
plt.close()