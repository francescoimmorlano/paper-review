...
for idx_take_out, (take_out, take_out_family) in enumerate(atmospheric_model_families_dict.items()):
    ...
    predictions_tl_on_simulations = read_tl_simulations_predictions_shuffle(idx_take_out, plot_figure_paper, take_out_family)

    if take_out_family:
        take_out_family_idx = []
        for member in take_out_family:
            take_out_family_idx.append(models_list_complete.index(member))
